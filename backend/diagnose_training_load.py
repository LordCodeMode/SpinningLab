#!/usr/bin/env python3
"""
Diagnostic script to test training load calculation and caching for User 2
...
def check_user_activities(db: Session):
    """Check User 2's activities and TSS values"""
    print_section("USER 2 ACTIVITIES CHECK")

    user = db.query(User).filter(User.email == "spotify.maxhartwig@googlemail.com").first()
    if not user:
        print("ERROR: User 2 (spotify.maxhartwig@googlemail.com) not found!")
        return None
    ...
    if not activities:
        print("ERROR: No activities found for User 2!")
        return user
...
def main():
    ...
    print(" User: spotify.maxhartwig@googlemail.com")
    print("="*60)

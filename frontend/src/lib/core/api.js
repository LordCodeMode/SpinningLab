...
export const API = {
  ...
  /**
   * Upload live training session to Strava
   */
  uploadStravaSession(session) {
    return apiClient.post('/api/strava/upload-session', session);
  },

  /**
   * Save live training session to dashboard
   */
  saveLiveSession(session) {
    return apiClient.post('/api/activities/live-session', session);
  },

...
class DataService {
  ...
  async rebuildBackendCache() {
    try {
      ...
      return {
        success: false,
        message: error?.message || 'Failed to trigger backend cache rebuild.'
      };
    }
  }
  ...
  async getFtpPredictions({ days = 90, forceRefresh = false } = {}) {
    const data = await this.getFtpPrediction({ days, forceRefresh });
  ...
  async getRecommendations({ days = 14, forceRefresh = false } = {}) {
    const data = await this.getInsights({ days, forceRefresh });
